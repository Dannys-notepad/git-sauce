...
`Git-sauce` is only available to Termux/Andriod users

As of now `Git-sauce v2.0 alpha` only has committing changes to remote repo feature
...
### Note:
`Git-sauce v2.0 alpha` still has other features under development, had to drop this version for anticipating users, and also next versions releases won't be for just Andriod/Termux users only
